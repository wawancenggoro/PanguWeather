...
def get_loss_synthetic(model, data_loader, device, loss1, loss2, lat_crop, lon_crop, world_size=4, pressure_weights_epoch=None, surface_weights_epoch=None):
    """
    ...
    static_plevel = np.random.rand(2, 1, 5, 13, 1, 1).astype(np.float32)
    ...
    static_slevel = np.random.rand(2, 1, 4, 1, 1).astype(np.float32)
    ...
    climatology_plevel = torch.tensor(np.random.rand(1, 5, 13, 1, 1).astype(np.float32)).to(device)
    climatology_slevel = torch.tensor(np.random.rand(1, 4, 1, 1).astype(np.float32)).to(device)
